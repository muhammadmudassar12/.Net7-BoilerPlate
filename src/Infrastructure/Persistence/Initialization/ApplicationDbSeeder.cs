using EMS20.WebApi.Infrastructure.Identity;
...
internal class ApplicationDbSeeder
{
    ...
    private async Task SeedRolesAsync(ApplicationDbContext dbContext)
    {
        foreach (string roleName in FSHRoles.DefaultRoles)
        {
            ...
            // Assign permissions
            if (roleName == FSHRoles.SuperAdmin)
            {
                await AssignPermissionsToRoleAsync(dbContext, FSHPermissions.SuperAdmin, role);
            ...
            else if (roleName == FSHRoles.Developer)
            {
                await AssignPermissionsToRoleAsync(dbContext, FSHPermissions.Developer, role);
            ...
            else if (roleName == FSHRoles.Trainer)
            {
                await AssignPermissionsToRoleAsync(dbContext, FSHPermissions.Trainer, role);
            ...
            else if (roleName == FSHRoles.Manager)
            {
            ...
            else if (roleName == FSHRoles.Supervisor)
            {
                await AssignPermissionsToRoleAsync(dbContext, FSHPermissions.Supervisor, role);
            ...
            else if (roleName == FSHRoles.User)
            {
                await AssignPermissionsToRoleAsync(dbContext, FSHPermissions.User, role);
